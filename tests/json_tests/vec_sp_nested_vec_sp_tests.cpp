...
TEST_CASE("prismJson - my_vec_sp elements each with their own my_vec_sp round trip", "[json][vec_sp][nested][combo]")
{
    SECTION("my_vec_sp element with its own vec_sp populated round trip")
    {
        tst_struct obj;
        obj.my_int = 1;
    ...
    SECTION("my_vec_sp two elements each with different inner vec_sp round trip")
    {
        tst_struct obj;
        obj.my_int = 2;
        obj.my_list_int.clear();
