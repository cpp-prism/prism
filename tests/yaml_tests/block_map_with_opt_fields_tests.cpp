...
TEST_CASE("prismYaml - block format my_map entries with optional fields in child structs round trip", "[yaml][block][map][opt][combo]")
{
    SECTION("my_map child with opt_str set block round trip")
    {
        tst_struct obj;
        obj.my_int = 1;
    ...
    SECTION("my_map two entries one with opt_int set one without block round trip")
    {
        tst_struct obj;
        obj.my_int = 2;
        obj.my_list_int.clear();
        ...
        REQUIRE(*result->my_map.at("with_opt").my_opt_int == 88);
        REQUIRE(!result->my_map.at("without_opt").my_opt_int.has_value());
    }

    SECTION("my_map child with bool and opt_str combined block round trip")
    {
        tst_struct obj;
        obj.my_int = 3;
        obj.my_list_int.clear();
        ...
        REQUIRE(*result->my_map.at("combo").my_opt_str == "block_opt");
    }
}
