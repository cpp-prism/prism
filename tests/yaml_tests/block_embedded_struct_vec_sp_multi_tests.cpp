...
TEST_CASE("prismYaml - block format my_struct.my_vec_sp multi-element round trip", "[yaml][block][struct][embedded][vector]")
{
    SECTION("my_struct.my_vec_sp with 3 elements block round trip")
    {
        tst_struct obj;
        obj.my_int = 10;
        ...
        for (int i = 1; i <= 3; ++i)
        {
            ...
            sub.my_int = i * 7;
            sub.my_bool = (i % 2 == 1);
            sub.my_string = "block_sub_" + std::to_string(i);
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            REQUIRE(result->my_struct.my_vec_sp[i].my_string == "block_sub_" + std::to_string(i + 1));
        }
    }

    SECTION("my_struct.my_vec_sp nested: element with its own my_vec_sp block round trip")
    {
        tst_struct obj;
        obj.my_int = 11;
        obj.my_list_int.clear();
        ...
        parent_sub.my_string = "block_parent";

        tst_sub_struct child_sub;
        child_sub.my_int = 51;
        child_sub.my_string = "block_child";
        ...
        REQUIRE(result->my_struct.my_vec_sp[0].my_string == "block_parent");
        REQUIRE(result->my_struct.my_vec_sp[0].my_vec_sp.size() == 1);
        REQUIRE(result->my_struct.my_vec_sp[0].my_vec_sp[0].my_int == 51);
        REQUIRE(result->my_struct.my_vec_sp[0].my_vec_sp[0].my_string == "block_child");
    }
}
