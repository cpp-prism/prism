...
TEST_CASE("prismYaml - block format multiple field combinations round trip", "[yaml][block][combo][struct]")
{
    SECTION("my_vec_sp + my_deque_int + my_set_str populated simultaneously block round trip")
    {
        tst_struct obj;
        obj.my_int = 1;
        ...
        REQUIRE(result->my_vec_sp[0].my_string == "block_combo_sub");
    ...
    SECTION("my_shared_sub + my_uptr_sub + my_opt_struct all populated block round trip")
    {
        tst_struct obj;
        obj.my_int = 2;
        obj.my_list_int.clear();
        ...
        obj.my_shared_sub->my_string = "block_shared_combo";

        obj.my_uptr_sub = std::make_unique<tst_sub_struct>();
        obj.my_uptr_sub->my_int = 22;
        obj.my_uptr_sub->my_string = "block_uptr_combo";

        tst_sub_struct opt_val;
        opt_val.my_int = 33;
        opt_val.my_string = "block_opt_combo";
        ...
        REQUIRE(result->my_shared_sub->my_string == "block_shared_combo");
        REQUIRE(result->my_uptr_sub != nullptr);
        REQUIRE(result->my_uptr_sub->my_int == 22);
        REQUIRE(result->my_uptr_sub->my_string == "block_uptr_combo");
        REQUIRE(result->my_opt_struct.has_value());
        REQUIRE(result->my_opt_struct->my_int == 33);
        REQUIRE(result->my_opt_struct->my_string == "block_opt_combo");
    }

    SECTION("my_vec_enum + my_map_enum + lang field block round trip")
    {
        tst_struct obj;
        obj.my_int = 3;
        obj.my_list_int.clear();
