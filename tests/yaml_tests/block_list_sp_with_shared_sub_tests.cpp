...
TEST_CASE("prismYaml - block format my_list_sp elements with my_shared_sub populated round trip", "[yaml][block][list][shared_ptr][struct]")
{
    SECTION("my_list_sp element has my_shared_sub non-null block round trip")
    {
        tst_struct obj;
        obj.my_int = 1;
        ...
        elem.my_shared_sub->my_string = "block_nested_shared_sub";
        ...
        REQUIRE(result->my_list_sp.front().my_shared_sub->my_string == "block_nested_shared_sub");
    }

    SECTION("my_list_sp 2 elements one with shared_sub one without block round trip")
    {
        tst_struct obj;
        obj.my_int = 2;
        obj.my_list_int.clear();
        ...
        elem1.my_shared_sub->my_int = 444;
        ...
        REQUIRE(it->my_shared_sub->my_int == 444);
        ++it;
