...
TEST_CASE("prismYaml - block format my_map entries alongside my_list_sp round trip", "[yaml][block][map][list][struct][nested]")
{
    SECTION("1 map entry and 2 list_sp elements block round trip")
    {
        tst_struct obj;
        obj.my_int = 1;
        ...
        map_val.my_string = "block_map_entry_1";
        ...
        lst1.my_string = "block_list_sp_1";
        ...
        lst2.my_string = "block_list_sp_2";
        ...
        REQUIRE(result->my_map["mk1"].my_string == "block_map_entry_1");
        REQUIRE(result->my_list_sp.size() == 2);
        auto it = result->my_list_sp.begin();
        REQUIRE(it->my_int == 20);
        ++it;
        REQUIRE(it->my_int == 30);
        REQUIRE(it->my_string == "block_list_sp_2");
    }

    SECTION("map entry with vec_enum and list_sp with nested sub block round trip")
    {
        tst_struct obj;
        obj.my_int = 2;
        obj.my_list_int.clear();
        ...
        tst_sub_struct sub;
        sub.my_int = 111;
        sub.my_string = "block_nested_sub_in_list";
        ...
        REQUIRE(result->my_map["mk2"].my_vec_enum.size() == 2);
        ...
        REQUIRE(result->my_list_sp.front().my_vec_sp[0].my_int == 111);
    }
}
