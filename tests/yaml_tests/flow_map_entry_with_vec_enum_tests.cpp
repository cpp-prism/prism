...
TEST_CASE("prismYaml - flow format my_map entries with my_vec_enum populated round trip", "[yaml][flow][map][struct][enum]")
{
    SECTION("my_map entry with my_vec_enum and lang flow round trip")
    {
        tst_struct obj;
        obj.my_int = 1;
        ...
        obj.my_map["flow_enum_key"] = std::move(map_val);
        ...
        REQUIRE(result->my_map["flow_enum_key"].my_int == 10);
        REQUIRE(result->my_map["flow_enum_key"].lang == language::TraditionalChinese);
        REQUIRE(result->my_map["flow_enum_key"].my_vec_enum.size() == 2);
        REQUIRE(result->my_map["flow_enum_key"].my_vec_enum[0] == language::english);
        REQUIRE(result->my_map["flow_enum_key"].my_vec_enum[1] == language::SimplifiedChinese);
    }

    SECTION("my_map 2 entries with different enum state flow round trip")
    {
        tst_struct obj;
        obj.my_int = 2;
        obj.my_list_int.clear();
