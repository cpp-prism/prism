...
template <class derived> struct Sql
{
    template <class T> static inline std::string deleteTable()
    {
        return derived::template deleteTable<T>();
    }

    template <class T> static inline std::string createTable()
    {
        return derived::template createTable<T>();
    }

    template <class T> static inline std::string queryTable(const char *where = nullptr)
    {
        return derived::template queryTable<T>(where);
    }

    template <class T> static inline std::string insert(std::vector<std::shared_ptr<T>> models)
    {
...
struct Sqlite3 : public Sql<Sqlite3>
{
    template <class T> static inline std::string deleteTable()
    {
        ...
        std::optional<const char *> tableName =
            prism::attributes::getClassAttr<T, sql::attributes::Attr_sql_class_alias>();
            ...
            *(T *)nullptr,
            [&](const char *fname, [[maybe_unused]] auto &&value)
            {
                std::optional<bool> ignore =
                    prism::attributes::getFieldAttr<T, sql::attributes::Attr_sql_field_ignore>(fname);
                ...
                std::optional<const char *> alias =
                    prism::attributes::getFieldAttr<T, sql::attributes::Attr_sql_field_alias>(fname);
                if (alias.has_value())
                    sql << alias.value();
                else
                    sql << fname;
            ...
            prism::reflection::for_each_fields(
                *item,
                [&](const char *fname, [[maybe_unused]] auto &&value)
                {
                    ...
                    std::optional<const char *> datatype =
                        prism::attributes::getFieldAttr<T, sql::sqlite3::attributes::Attr_sql_field_datatype>(fname);
        ...
        prism::reflection::for_each_fields(
            *((T *)nullptr),
            [&](const char *fname, [[maybe_unused]] auto &&value)
            {
                ...
                std::optional<const char *> alias_opt =
                    prism::attributes::getFieldAttr<T, sql::attributes::Attr_sql_field_alias>(fname);
                if (alias_opt.has_value())
                    alias = alias_opt.value();

                std::optional<bool> primaryKey =
                    prism::attributes::getFieldAttr<T, sql::attributes::Attr_sql_field_isPrimaryKey>(fname);
            ...
            *((T *)nullptr),
            [&](const char *fname, [[maybe_unused]] auto &&value)
            {
                ...
                else
                {
                    ...
                    //         std::optional<const char* > alias =
                    //         prism::attributes::getFieldAttr<b_t,sql::attributes::Attr_sql_field_alias>(fname);
                    //         if(alias.has_value())
                    //         {
                    //             sql << alias.value();
                    //             hited = true;
...
//enum -> sql string type
    ...
    namespace privates
    {
        ...
        struct
field_attribute<T,std::enable_if<prism::utilities::has_def<prism::enums::enum_info<M>>::value,M>,fn,::prism::sql::sqlite3::attributes::Attr_sql_field_datatype>
        {
            constexpr static std::stdoptional<::prism::sql::sqlite3::attributes::Attr_sql_field_datatype::value_type>
value()
            {
            return "TEXT";
            }
